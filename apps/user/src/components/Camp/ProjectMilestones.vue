<template>
  <!-- 项目里程碑交付区（v1.3 阶段4）：节点时间轴 + 版本化提交 + 双模式审核。
       team 节点=负责人统一提交老师审；member 节点=成员各交负责人审（负责人份额老师审）。
       嵌在 ProjectHub 的项目卡内使用。 -->
  <div class="pm-wrap">
    ...
    <template v-else>
      ...
      <!-- 节点列表 -->
      <div v-for="m in milestones" :key="m.id" :class="['ms-row', `ms-${m.status}`]">
        <button type="button" class="ms-head" @click="toggleExpand(m.id)">
          ...
          <DewTag v-if="m.submit_mode === 'member'" size="sm" round>个人交付</DewTag>
          <DewTag v-else size="sm" round>整队交付</DewTag>
          <span v-if="m.due_date" class="ms-due">{{ m.due_date }}</span>
          <span :class="['ms-status', `st-${m.status}`]">{{ STATUS_TEXT[m.status] || m.status }}</span>
...
const props = defineProps({
  ...
  ownerUserId: { type: Number, default: null },   // 项目负责人（审核人判定用）
});
...
.ms-row.ms-approved { border-color: color-mix(in srgb, var(--color-success) 35%, transparent); }
.ms-head {
...
.ms-approved .ms-order { color: var(--color-success); border-color: color-mix(in srgb, var(--color-success) 40%, transparent); }
.ms-title { font-weight: 650; color: var(--dew-text-heading); }
.ms-due { font-size: 12px; color: var(--dew-text-faint); }
.ms-status { font-size: 12px; font-weight: 600; margin-left: auto; }
